...
void V_CalcViewLag(struct ref_params_s *pparams)
{
	...
	if (cl_viewlag->value > 0.0f)
	{
		...
		VectorMA(Vector(pparams->vieworg), 2.0f, vecDiff, newOrigin);
		VectorMA(Vector(newOrigin), -pitch * 0.035f, forward, newOrigin);
		VectorMA(Vector(newOrigin), -pitch * 0.03f, right, newOrigin);
		VectorMA(Vector(newOrigin), -pitch * 0.02f, up, newOrigin);
	...
	if (cl_vsmoothing && cl_vsmoothing->value && (pparams->smoothing && (pparams->maxclients > 1)))
	{
		...
		if (i < ORIGIN_MASK && ViewInterp.OriginTime[foundidx & ORIGIN_MASK] != 0.0)
		{
			...
			if (dt > 0.0)
			{
				...
				VectorMA(Vector(ViewInterp.Origins[foundidx & ORIGIN_MASK]), frac, delta, neworg);
