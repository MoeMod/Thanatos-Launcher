...
#if 0
#define USE_OLDRADARCODE
#endif

void __fastcall CHudHealth_DrawRadar(void *pthis, int, float flTime)
{
#ifdef USE_OLDRADARCODE
	g_pfnCHudHealth_DrawRadar(pthis, 0, flTime);
#endif
}

int CHudRadar::Init(void)
{
	g_pMetaHookAPI->InlineHook(g_pfnCHudHealth_DrawRadar, CHudHealth_DrawRadar, (void *&)g_pfnCHudHealth_DrawRadar);
...
{
	return gEngfuncs.Con_Printf("Missing command: trackplayer\n");
...
{
	return gEngfuncs.Con_Printf("Missing command: clearplayers\n");
...
{
	if (cl_newradar->value && g_MapSprite)
		return;

#ifndef USE_OLDRADARCODE
	Vector vPlayer;
