...
{
	gWR.SelectSlot(iSlot, fAdvance, iDirection);
}
...
int __fastcall CHudAmmo_Draw(void *pthis, int, float flTime)
{
	return 1;
}
...
int CHudAmmo::Init(void)
{
	...
	g_pMetaHookAPI->InlineHook(g_pfnWeaponsResource_SelectSlot, WeaponsResource_SelectSlot, (void *&)g_pfnWeaponsResource_SelectSlot);
	g_pMetaHookAPI->InlineHook(g_pfnCHudAmmo_Draw, CHudAmmo_Draw, (void *&)g_pfnCHudAmmo_Draw);
...
void WeaponsResource::SelectSlot(int iSlot, int fAdvance, int iDirection)
{
	if (!cl_newmenu->value)
	{
		if ((*g_pgHUD_m_Menu_m_fMenuDisplayed) && (fAdvance == FALSE) && (iDirection == 1))
		{
			g_pfnCHudMenu_SelectMenuItem(g_pgHUD_m_Menu, 0, iSlot + 1);
			return;
		}
	}
	else
...
#if 0
#define TIMETEST_CALCCROSSHAIRCOLOR
#endif
