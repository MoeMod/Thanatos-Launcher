...
void COptionsSubVideo::GetVidSettings()
{
	...
	if (g_bIsNewEngine)
	{
		class INewGameUIFuncs : public IBaseInterface
		{
		public:
		...
		INewGameUIFuncs *new_gameuifuncs = reinterpret_cast<INewGameUIFuncs *>(gameuifuncs);
		new_gameuifuncs->GetCurrentRenderer(p->renderer, 128, &p->windowed, &p->hdmodels, &p->addons_folder, &p->vid_level);
	}
	else
