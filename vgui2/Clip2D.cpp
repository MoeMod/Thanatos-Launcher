...
template <class Clipper> static inline void Intersect(const vgui::Vertex_t &start, const vgui::Vertex_t &end, vgui::Vertex_t *pOut, Clipper &clipper)
{
...
template<class Clipper> bool ClipLineToPlane(Clipper &clipper, const vgui::Vertex_t *pInVerts, vgui::Vertex_t *pOutVerts)
{
...
{
	if (IsX360() && (!g_bScissor || g_bFullScreenScissor || (inUL.m_Position.x >= g_ScissorRect.left && inLR.m_Position.x <= g_ScissorRect.right && inUL.m_Position.y >= g_ScissorRect.top && inLR.m_Position.y <= g_ScissorRect.bottom)))
	{
		*pOutUL = inUL;
		*pOutLR = inLR;
		return true;
