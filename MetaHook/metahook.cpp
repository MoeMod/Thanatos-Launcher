...
void MH_Init(const char *pszGameName)
{
	...
	g_pPlugins = (IPlugins *)fnCreateInterface(METAHOOK_PLUGIN_API_VERSION, NULL);
	g_pPlugins->Init(&gMetaHookAPI, &gInterface, &gMetaSave);
}
...
int ClientDLL_Initialize(struct cl_enginefuncs_s *pEnginefuncs, int iVersion)
{
	...
	g_pPlugins->LoadClient(g_pExportFuncs);
...
void MH_LoadEngine(HMODULE hModule)
{
	...
	g_pPlugins->LoadEngine();
}

void MH_ExitGame(int iResult)
{
	g_pPlugins->ExitGame(iResult);
}
