...
HINTERFACEMODULE LoadFilesystemModule(void)
{
	...
	if (!hModule)
	{
		MessageBox(NULL, "载入文件系统失败，请把本启动器放到CS目录中运行。", "Fatal Error", MB_ICONERROR);
		return NULL;
	}
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	CommandLine()->RemoveParm("-16bpp");
	CommandLine()->AppendParm("-32bpp", NULL);
	...
	if (!IsDebuggerPresent() && CommandLine()->CheckParm("-nomutex") == NULL)
	{
		...
		{
			MessageBox(NULL, "检测到其他的游戏实例已打开。\n本游戏不允许多开。", "Error", MB_ICONERROR);
			return 0;
		}
	}
#endif
	...
	if (registry->ReadInt("CrashInitializingVideoMode", FALSE))
	{
		...
		if (strcmp(registry->ReadString("EngineDLL", "hw.dll"), "hw.dll"))
		{
			...
			registry->WriteInt("ScreenWidth", 1024);
			registry->WriteInt("ScreenHeight", 768);
			registry->WriteInt("ScreenBPP", 32);
		}
	}

	if (registry->ReadString("ValveKey", NULL) == NULL)
	{
		registry->WriteString("ValveKey", "5RP2E-EPH3K-BR3LG-KMGTE-FN8PY");
	}
