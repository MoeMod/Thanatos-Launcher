...
hook_t *g_phInfo_SetValueForKey;
...
void Info_RemoveKey(char *s, char *key)
{
	...
	if (strstr(key, "\\"))
	{
		gEngfuncs.Con_Printf("Can't use a key with a \\\n");
		return;
	}
...
void Info_SetValueForStarKey(char *s, char *key, char *value, int maxsize)
{
	...
	{
		gEngfuncs.Con_Printf("Can't use keys or values with a \\\n");
		return;
	}
	...
	{
		gEngfuncs.Con_Printf("Keys and values must be < %i characters.\n", MAX_KV_LEN + 1);
		return;
	}
	...
	if (strlen(_new) + strlen(s) >= (size_t)maxsize)
	{
		if (Info_IsKeyImportant(key))
		{
			...
			if (!largekey[0])
			{
				gEngfuncs.Con_Printf("Info string length exceeded\n");
				return;
			}
		}
		else
...
void Info_Print(char *s)
{
	...
	while (*s)
	{
		...
		gEngfuncs.Con_Printf("%s", key);

		if (!*s)
		{
			gEngfuncs.Con_Printf("MISSING VALUE\n");
			return;
		}
...
void Info_SetValueForKey(char *s, char *key, char *value, int maxsize)
{
	if (key[0] == '*')
	{
		gEngfuncs.Con_Printf("Can't set * keys\n");
		return;
	}
