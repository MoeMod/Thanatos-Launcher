...
void CSocketThread::InitTimer(void)
{
	...
	if (!m_nTimeSampleShift)
		m_uiPreviousTime = (unsigned int)PerformanceCount.LowPart;
	else
		m_uiPreviousTime = ((unsigned int)PerformanceCount.LowPart >> m_nTimeSampleShift) | ((unsigned int)PerformanceCount.HighPart << (32 - m_nTimeSampleShift));
}
